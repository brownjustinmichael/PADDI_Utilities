...

{
  ...
  start_of_step = ftello(fpo[*iu]);
   printf(" am anfang   start_of_step %i   \n", start_of_step);
  ...
  fprintf(fpo[*iu],"%8.8i\n", 0); /** only a dummy */
   ...
   printf("  writing subimage   start_of_step %i  start_of_joep %i   \n", start_of_step, start_of_joep);
...
//   for(i=0; i< 30 ; i++) fprintf(fpo[*iu], "C");
  ...
  fprintf(fpo[*iu],"%8.8i\n", length_of_joep);                /**length info at beginning of step **/
   ...
   printf(">>>>>>>> jumping to %i and writing %i \n", start_of_joep, length_of_joep);
